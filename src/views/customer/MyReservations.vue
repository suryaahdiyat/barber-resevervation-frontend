<template>
  <div class="max-w-md md:max-w-2xl lg:max-w-4xl w-full mx-auto p-4">
    <p class="dark:text-slate-100 text-slate-950 text-lg font-bold mb-2">Reservasi Saya</p>
    ...
    <div class="max-w-md md:max-w-2xl mx-auto lg:max-w-4xl mb-2">
      ...
      <div class="flex gap-2 flex-wrap mt-2">
        <button
          ...
          :class="selectedStatus === paymentStatus.key ? paymentStatus.active : paymentStatus.inactive"
        >
          {{ paymentStatus.label }}
        </button>
      </div>
    ...
    <div v-if="reservations.length > 0" class="space-y-2">
      ...
      >
        <div class="mt-1 px-1 flex text-start w-full justify-between items-center">
          ...
          <div class="w-1/4 text-xs flex justify-center gap-2 items-center" :class="`text-${statusColors[r.payment.status]}`">
            <div class="h-1 w-1 rounded-full" :class="`bg-${statusColors[r.payment.status]}`"></div>
...
const fetchReservations = async () => {
  ...
  //nanti diambil dari data login
  const idUser = 5;
  try {
    const res = await api.get(`/reservations/user/${idUser}`, {
      params: {
...
const statusColors = {
  waiting:  "amber-500",
  accepted: "emerald-600",
  rejected: "red-600",
  refund_pending: "orange-500",
  refunded: "purple-400"
};
